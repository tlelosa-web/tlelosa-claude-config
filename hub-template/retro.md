...
> **`retro-log.md` is a contention file — pull before appending.** It joins
> the task queue, the session log, and any knowledge-cache index as a file
> written by whichever session happens to run, from any machine or cloud
> container, concurrently. Those files have already caused real merge
> conflicts in at least one vault. `git fetch` + pull immediately before the
> append, not once at the start of the run, and add `retro-log.md` to
> whatever contention-file list this vault's `continue.md` (Step 1.75) and
> hard rules already name. Resolve any conflict as a real union of both
> sides — never keep one run's entry and discard the other's.
